...
分类任务中的三种情况为：分类欠妥、正确的分类、分类过度。由于分类可以看作是对分类边界的拟合，所以我们经常也统称其为拟合。
...
![](./IMGs/overfitting_sin_loss.png)

拟合情况：

![](./IMGs/overfitting_sin_result.png)
...
![](./IMGs/L2_sin_loss.png)

![](./IMGs/L2_sin_result.png)
...
### 五、总结
这次课学习正则化的相关知识，即Regularization，它是用于防止过拟合的。回归任务中的三种情况，依次为：欠拟合、正确的拟合、过拟合，正是由于遇到这些情况，所以需要进行正则化的应用。

另外，我们有学习了L2、L1正则化，L2范数是指向量各元素的平方和然后求平方根，L1范数是指向量中各个元素绝对值之和，也叫“稀疏规则算子”（Lasso regularization）。这两个正则化对于解决拟合问题有很大的帮助，绝大多数算法都有数学知识的支撑，数学的重要性由此可见。

通过代码实现和比较结果，可以发现L1正则的效果就是让权重参数矩阵稀疏化，以形成特征选择。L2正则化的结果，是使得权重矩阵中的值普遍减小，拉向坐标原点。权重值变小，就会对特征不敏感，大部分特征都能起作用时。L2、L1各有一定的优势，具体应用时应该根据问题的需要进行选择。不同的正则化就会产生不一样的效果。